...
DATA_PATH = 'data2'
# Clean up folders
rmdir(DATA_PATH + '/whoas/traditional_images_background')
rmdir(DATA_PATH + '/whoas/traditional_images_evaluation')
mkdir(DATA_PATH + '/whoas/traditional_images_background')
mkdir(DATA_PATH + '/whoas/traditional_images_evaluation')
...
for i in range(len(species)):
    images.append([])
    for _,_,files in walk(DATA_PATH + '/whoas/images/' + species[i]):
        #print(folders)
        for f in files:
            #print(f)
            images[i].append(f)
...
for folderName in species:
    mkdir(DATA_PATH + '/whoas/traditional_images_background/' + folderName)
for folderName in species:
    mkdir(DATA_PATH + '/whoas/traditional_images_evaluation/' + folderName)

print('Preparing background data....')
for i in tqdm(range(len(images))):
    folder = species[i]
    for j in range(len(images[i][:image_split_list[i]])):
        src = DATA_PATH + '/whoas/images/' + folder + '/' + images[i][j]
        dst = dst = DATA_PATH + '/whoas/traditional_images_background/' + folder + '/'# + images[i][j]
        shutil.copy(src,dst)
...
for i in tqdm(range(len(images))):
    ...
    for j in range(len(images[i][image_split_list[i]:])):
        src = DATA_PATH + '/whoas/images/' + folder + '/' + images[i][j]
        dst = dst = DATA_PATH + '/whoas/traditional_images_evaluation/' + folder + '/' + images[i][j]
        shutil.copy(src,dst)
'''
np.random.seed(0)
